...
/* =========================================================
   FIELD TANGGAL + KALENDER SENDIRI
   Popup kalender bawaan browser tidak dapat diatur lewat CSS,
   jadi input date diganti input teks dd-mm-yyyy plus tombol
   kalender, dan popup kalendernya dirender sendiri. Nilai yang
   dikirim ke server tetap disimpan sebagai Y-m-d pada input
   tersembunyi #ajbTglAwal dan #ajbTglAkhir.
   ========================================================= */
.ajb-datefield {
    position: relative;
    min-width: 0;
...
.ajb-date-button:hover {
    background: #2563eb;
    color: #ffffff;
}

.ajb-datefield.is-open .ajb-date-button {
    background: #2563eb;
    color: #ffffff;
}

.ajb-datefield.is-open .ajb-input {
    border-color: #2563eb;
    box-shadow: 0 0 0 3px rgba(37, 99, 235, 0.13);
}
...
.ajb-datefield.is-open .ajb-cal {
    display: block;
    animation: ajbCalIn 0.14s ease-out;
}
...
.ajb-cal-day.is-outside {
    color: #cbd5e1;
    font-weight: 600;
}

.ajb-cal-day.is-weekend {
    color: #ef4444;
}

.ajb-cal-day.is-outside.is-weekend {
    color: #fecaca;
}

.ajb-cal-day.is-today {
    box-shadow: inset 0 0 0 1.5px #bfdbfe;
    color: #1d4ed8;
}

.ajb-cal-day.is-selected,
.ajb-cal-day.is-selected:hover {
...
.ajb-cal-action:hover {
    border-color: #bfdbfe;
    background: #eff6ff;
    color: #1d4ed8;
}

.ajb-cal-action--primary {
    border-color: #2563eb;
    background: linear-gradient(135deg, #2563eb, #1d4ed8);
    color: #ffffff;
}

.ajb-cal-action--primary:hover {
    border-color: #1d4ed8;
    background: linear-gradient(135deg, #1d4ed8, #1e40af);
    color: #ffffff;
}

/* Kalender pada field kanan dirapatkan ke kanan agar tidak keluar kartu. */
.ajb-datefield--akhir .ajb-cal {
    left: auto;
    right: 0;
}
    ...
    <section class="ajb-filter">
        ...
        <div class="ajb-filter-grid">
            ...
            <div class="ajb-field">
                ...
                <div class="ajb-range">
                    <div class="ajb-datefield" id="ajbDateAwal">
                        <input type="hidden" id="ajbTglAwal" value="">
                        <input
                            type="text"
                            id="ajbTglAwalText"
                            class="ajb-input"
                        ...
                        <button
                            type="button"
                            class="ajb-date-button"
                            onclick="toggleAjbCalendar('awal', event)"
                            title="Pilih tanggal"
                            aria-label="Pilih tanggal awal"
                        >
                            <i class="fas fa-calendar-alt"></i>
                        </button>
                        <div class="ajb-cal" id="ajbCalAwal"></div>
                    </div>

                    <span class="ajb-separator">s.d</span>

                    <div class="ajb-datefield ajb-datefield--akhir" id="ajbDateAkhir">
                        <input type="hidden" id="ajbTglAkhir" value="">
                        <input
                            type="text"
                            id="ajbTglAkhirText"
                            class="ajb-input"
                        ...
                        <button
                            type="button"
                            class="ajb-date-button"
                            onclick="toggleAjbCalendar('akhir', event)"
                            title="Pilih tanggal"
                            aria-label="Pilih tanggal akhir"
                        >
                            <i class="fas fa-calendar-alt"></i>
                        </button>
                        <div class="ajb-cal" id="ajbCalAkhir"></div>
                    </div>
                </div>
            </div>
...
<script>
    ...
    $(document).ready(function () {
        ...
        $(document).on('click', function (event) {
            ...
            if (!$(event.target).closest('.ajb-datefield').length) {
                closeAjbCalendar();
            }
        });

        $('#ajbTglAwalText').on('change blur', function () {
            commitAjbDateText('awal');
        });

        $('#ajbTglAkhirText').on('change blur', function () {
            commitAjbDateText('akhir');
        });

        $('#ajbTglAwalText, #ajbTglAkhirText').on('focus', function () {
            closeAjbCalendar();
        });
    });
    ...
    /* ==============================================
       ...
       Popup kalender bawaan browser tidak dapat diatur lewat CSS,
       jadi kalendernya dirender sendiri. Input yang terlihat berisi
       teks dd-mm-yyyy, sedangkan nilai yang dikirim ke server tetap
       Y-m-d pada #ajbTglAwal dan #ajbTglAkhir.
       ============================================== */
    ...
    var AJB_NAMA_HARI = ['Min', 'Sen', 'Sel', 'Rab', 'Kam', 'Jum', 'Sab'];

    /* Bulan yang sedang ditampilkan tiap kalender, format Y-m. */
    var ajbCalView = { awal: null, akhir: null };

    function ajbPad2(angka) {
        return String(angka).padStart(2, '0');
    }

    function ajbDateToIso(date) {
        return date.getFullYear()
            + '-' + ajbPad2(date.getMonth() + 1)
            + '-' + ajbPad2(date.getDate());
    }

    function ajbIsoToDate(iso) {
        var cocok = String(iso || '').match(/^(\d{4})-(\d{2})-(\d{2})$/);

        if (!cocok) {
            return null;
        }

        var tahun = Number(cocok[1]);
        var bulan = Number(cocok[2]) - 1;
        var tanggal = Number(cocok[3]);
    ...
    function ajbIsoToDisplay(iso) {
        var date = ajbIsoToDate(iso);

        if (!date) {
            return '';
        }

        return ajbPad2(date.getDate())
            + '-' + ajbPad2(date.getMonth() + 1)
            + '-' + date.getFullYear();
    }

    /* Menerima dd-mm-yyyy dan dd/mm/yyyy. */
    function ajbDisplayToIso(teks) {
        var cocok = String(teks || '')
            .trim()
        ...
        var iso = cocok[3]
            + '-' + ajbPad2(Number(cocok[2]))
            + '-' + ajbPad2(Number(cocok[1]));

        return ajbIsoToDate(iso) ? iso : '';
    }

    function setAjbDate(which, iso) {
        $('#ajbTgl' + (which === 'akhir' ? 'Akhir' : 'Awal')).val(iso || '');
        $('#ajbTgl' + (which === 'akhir' ? 'Akhir' : 'Awal') + 'Text')
            .val(ajbIsoToDisplay(iso));
    }

    function getAjbDate(which) {
        return String(
            $('#ajbTgl' + (which === 'akhir' ? 'Akhir' : 'Awal')).val() || ''
        );
    }

    /*
     * Teks yang diketik manual dipindahkan ke nilai tersembunyi. Bila
     * formatnya tidak dikenali, teks dikembalikan ke nilai terakhir yang
     * sah supaya isian tidak pernah menjadi setengah jadi.
     */
    function commitAjbDateText(which) {
        var $teks = $('#ajbTgl' + (which === 'akhir' ? 'Akhir' : 'Awal') + 'Text');
        var iso = ajbDisplayToIso($teks.val());

        if (iso) {
            setAjbDate(which, iso);
            return;
        }

        setAjbDate(which, getAjbDate(which));
    }

    function setAjbDefaultDate() {
        var today = ajbDateToIso(new Date());

        setAjbDate('awal', today);
        setAjbDate('akhir', today);
    }

    function toggleAjbCalendar(which, event) {
        if (event) {
            event.preventDefault();
            event.stopPropagation();
        }

        var $field = $('#ajbDate' + (which === 'akhir' ? 'Akhir' : 'Awal'));
        var akanDibuka = !$field.hasClass('is-open');
        ...
        var terpilih = getAjbDate(which);
        var acuan = ajbIsoToDate(terpilih) || new Date();

        ajbCalView[which] = acuan.getFullYear() + '-' + ajbPad2(acuan.getMonth() + 1);

        $field.addClass('is-open');
        renderAjbCalendar(which);
    }

    function closeAjbCalendar() {
        $('.ajb-datefield').removeClass('is-open');
    }

    function shiftAjbCalendar(which, selisihBulan) {
        var bagian = String(ajbCalView[which] || '').split('-');
        var acuan = new Date(
            Number(bagian[0]),
            Number(bagian[1]) - 1 + selisihBulan,
            1
        );
    ...
    function setAjbCalendarPeriod(which) {
        var $cal = $('#ajbCal' + (which === 'akhir' ? 'Akhir' : 'Awal'));
        var bulan = Number($cal.find('.ajb-cal-month').val());
        var tahun = Number($cal.find('.ajb-cal-year').val());

        ajbCalView[which] = tahun + '-' + ajbPad2(bulan + 1);
        renderAjbCalendar(which);
    }

    function pickAjbCalendar(which, iso) {
        setAjbDate(which, iso);
        closeAjbCalendar();
        resetAjbPrint();
    }

    function pickAjbCalendarToday(which) {
        pickAjbCalendar(which, ajbDateToIso(new Date()));
    }

    function renderAjbCalendar(which) {
        var $cal = $('#ajbCal' + (which === 'akhir' ? 'Akhir' : 'Awal'));
        var bagian = String(ajbCalView[which] || '').split('-');
        var tahunAktif = Number(bagian[0]);
        var bulanAktif = Number(bagian[1]) - 1;

        var terpilih = getAjbDate(which);
        var hariIni = ajbDateToIso(new Date());

        var pertama = new Date(tahunAktif, bulanAktif, 1);
        var mulai = new Date(tahunAktif, bulanAktif, 1 - pertama.getDay());

        var html = '';

        html += '<div class="ajb-cal-head">';
        html += '<button type="button" class="ajb-cal-nav" ';
        html += 'onclick="shiftAjbCalendar(\'' + which + '\', -1)" ';
        html += 'aria-label="Bulan sebelumnya">&#9664;</button>';

        html += '<div class="ajb-cal-period">';
        html += '<select class="ajb-cal-select ajb-cal-month" ';
        html += 'onchange="setAjbCalendarPeriod(\'' + which + '\')">';

        for (var b = 0; b < 12; b++) {
            html += '<option value="' + b + '"'
                + (b === bulanAktif ? ' selected' : '') + '>'
                + AJB_NAMA_BULAN[b] + '</option>';
        }

        html += '</select>';
        html += '<select class="ajb-cal-select ajb-cal-year" ';
        html += 'onchange="setAjbCalendarPeriod(\'' + which + '\')">';

        var tahunMulai = Math.min(tahunAktif, new Date().getFullYear() - 15);
        var tahunSelesai = Math.max(tahunAktif, new Date().getFullYear() + 5);

        for (var t = tahunMulai; t <= tahunSelesai; t++) {
            html += '<option value="' + t + '"'
                + (t === tahunAktif ? ' selected' : '') + '>'
                + t + '</option>';
        }
        ...
        html += '<button type="button" class="ajb-cal-nav" ';
        html += 'onclick="shiftAjbCalendar(\'' + which + '\', 1)" ';
        html += 'aria-label="Bulan berikutnya">&#9654;</button>';
        html += '</div>';

        html += '<div class="ajb-cal-grid">';

        $.each(AJB_NAMA_HARI, function (index, nama) {
            html += '<div class="ajb-cal-dow'
        ...
        for (var i = 0; i < 42; i++) {
            ...
            var iso = ajbDateToIso(tanggal);
            var luarBulan = tanggal.getMonth() !== bulanAktif;
            var akhirPekan = tanggal.getDay() === 0 || tanggal.getDay() === 6;

            var kelas = 'ajb-cal-day';
            if (luarBulan) { kelas += ' is-outside'; }
            if (akhirPekan) { kelas += ' is-weekend'; }
            if (iso === hariIni) { kelas += ' is-today'; }
            if (iso === terpilih) { kelas += ' is-selected'; }

            html += '<button type="button" class="' + kelas + '" ';
            html += 'onclick="pickAjbCalendar(\'' + which + '\', \'' + iso + '\')">';
            html += tanggal.getDate();
            html += '</button>';
        }

        html += '</div>';

        html += '<div class="ajb-cal-foot">';
        html += '<button type="button" class="ajb-cal-action" ';
        html += 'onclick="closeAjbCalendar()">Tutup</button>';
        html += '<button type="button" class="ajb-cal-action ajb-cal-action--primary" ';
        html += 'onclick="pickAjbCalendarToday(\'' + which + '\')">Hari Ini</button>';
        html += '</div>';
