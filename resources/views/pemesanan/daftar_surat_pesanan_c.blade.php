...
<style>
    ...
    #noDataAlertModal .alert-icon-wrapper {
        width: 64px;
        height: 64px;
        margin: 0 auto 16px;
        display: flex;
    ...
    #noDataAlertModal .alert-message {
        color: #475569;
        font-size: 14px;
        margin-bottom: 24px;
        font-family: "Segoe UI", Tahoma, Arial, sans-serif;
    }
    #noDataAlertModal .alert-btn-ok {
        background: linear-gradient(135deg, #2563eb, #1d4ed8);
        color: #fff;
        border: none;
        border-radius: 12px;
    ...
    #noDataAlertModal .alert-btn-ok:hover {
        transform: translateY(-1px);
        ...
        <div class="modal-dialog modal-dialog-centered"> 
            <div class="modal-content">
                <div class="modal-body text-center p-4">
                    ...
                    <div class="alert-message">Data Uang Muka periode ini tidak ada.....!</div>
                    <button type="button" class="btn alert-btn-ok" onclick="$('#noDataAlertModal').modal('hide')">OK</button>
                </div>
            </div>
        </div>
    </div>
...
<div class="sris-page-header">
    <span class="sris-page-header-title">Daftar Surat Pesanan</span>
            ...
            success: function (data) {
                ...
                if (!data || data.length < 1) {
                    ...
                    $.each(data, function (index, item) {
                        ...
                        var npwp = item.NPWP == null ? '' : item.NPWP;
                        var namaPj = item.NAMA_PJ == null ? '' : item.NAMA_PJ;

                        html += '<tr onclick="add_agen(\'' + escapeJs(kode) + '\', \'' + escapeJs(namaAgen) + '\')">';
                        html += '<td>' + escapeHtml(namaAgen) + '</td>';
                        html += '<td>' + escapeHtml(kode) + '</td>';
                        html += '<td>' + escapeHtml(namaPt) + '</td>';
                        html += '<td>' + escapeHtml(npwp) + '</td>';
                        html += '<td>' + escapeHtml(namaPj) + '</td>';
                        html += '</tr>';
                    });
