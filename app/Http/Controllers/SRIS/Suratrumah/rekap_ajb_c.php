...
{
    protected rekap_ajb_m $model;

    public function __construct()
    {
        $this->model = new rekap_ajb_m();
    }

    public function viewRekapAktaJualBeli()
    {
        return view('SRIS.Suratrumah.rekap_ajb_v');
    }
    ...
    public function obtainRekapAktaJualBeli(Request $request)
    {
        $this->validateFilter($request);

        return response()->json(
            $this->model->obtainRekapAktaJualBeli($request)
        );
    }
    ...
    private function validateFilter(Request $request): void
    {
        $request->validate([
            ...
            'belum_ttd_akta' => ['nullable', 'in:Y,T,1,0'],
        ]);
    }
}
